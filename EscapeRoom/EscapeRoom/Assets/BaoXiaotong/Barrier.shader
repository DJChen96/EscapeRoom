...
Shader "CloverSwatch/BinstonBarrier"
{
	...
	SubShader
	{
		...
		Pass
		{
			...
			{
				return saturate(abs(frac(offset + t) * 2 - 10) + yOffset);
			}

			fixed4 texColor(v2f i, float rim)
			{
				fixed4 mainTex = tex2D(_MainTex, i.uv);
				mainTex.r *= triWave(_Time.x * 5, abs(i.objectPos.y) * 2, -0.7) * 40;
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				float rim = 1 - abs(dot(i.normal, normalize(i.viewDir))) * 8;
